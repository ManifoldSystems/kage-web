...
import { adapters, agents, decision, links, runEvents, stages, traceEvents, type TraceCategory } from "../data/kage";
...
// The loop as the README draws it: PLAN -> EXECUTE -> TEST -> REVIEW -> DECISION, with FIX
// returning to TEST. TEST runs before REVIEW, so a broken build never reaches the reviewer.
const heroPath = ["PLAN", "EXECUTE", "TEST", "REVIEW", "DECISION", "FIX"];
const nodeForStep = [0, 1, 2, 3, 4, 4, 5, 2, 3, 4, 4, 4];
export function HeroLoop() {
  const reduced = useReducedMotion();
  const [step, setStep] = useState(reduced ? 15 : 0);
  const states = ["PLAN", "EXECUTE", "TEST", "REVIEW", "DECISION", "FAILED", "FIX", "TEST", "REVIEW", "DECISION", "PASSED", "COMPLETED"];
  useEffect(() => {
  ...
  return <div className="hero-loop" aria-label="Engineering loop: plan, execute, test, review, decision. A failing review sends the run through FIX and back to TEST, and a second review passes and completes the run">
    <div className="loop-center"><span>ITERATION</span><strong>{step < 6 ? "01" : "02"}</strong><em className={current === "FAILED" ? "fail" : current === "PASSED" || current === "COMPLETED" ? "pass" : ""}>{current}</em></div>
    <div className="orbit-line" />
    {heroPath.map((name, i) => <div key={name} className={`orbit-node n${i} ${current === name ? "active" : ""}`}><span>0{i + 1}</span>{name}</div>)}
    <div className={`token t${nodeForStep[step] ?? 4}`} />
    <div className={`outcome fail-outcome ${current === "FAILED" ? "show" : ""}`}>REVIEW FAILED → FIX ↺</div>
    <div className={`outcome pass-outcome ${current === "COMPLETED" ? "show" : ""}`}><Check /> COMPLETED</div>
  </div>;
}
...
export function RunDemo() {
  ...
  return <section className="run-section shell" aria-labelledby="run-title"><div className="terminal run-terminal">
    <div className="terminal-bar"><div><span className="live-dot" /> kage run_20260809_001</div><div className="controls"><button onClick={() => setPlaying(!playing)} disabled={visible >= runEvents.length} aria-label={playing ? "Pause run demo" : "Resume run demo"}>{playing ? <Pause /> : <Play />}{playing ? "Pause" : "Resume"}</button><button onClick={replay}><RotateCcw /> Replay</button></div></div>
    <div className="run-objective"><span>TASK</span><h2 id="run-title">Add configurable API rate limiting.</h2></div>
...
export function Architecture() {
  ...
  <div className="orchestrator"><div className="kage-core"><span>KAGE</span><strong>ENGINEERING<br />ORCHESTRATOR</strong></div><ArrowDown /><div className="backend-row">{["PLANNER", "EXECUTOR", "REVIEWER"].map((x, i) => <div key={x}><strong>{x}</strong><ArrowDown /><span>{i === 1 ? "Coding agent CLI" : "Coding agent CLI or HTTP endpoint"}<small>REPLACEABLE</small></span></div>)}</div><p>The workflow never names a model. <b>Binding a role to a harness is a config edit, not a code change.</b></p></div>
  <p className="callout">{decision}</p></>;
}
...
export function Footer() {
  return <footer><div className="shell footer-grid"><div><strong className="footer-logo">KAGE</strong><p>Engineering workflow orchestrator for AI coding agents.</p></div><nav aria-label="Footer navigation"><a href={links.docs}>Docs</a><a href={links.architecture}>Architecture</a><a href={links.github}>GitHub</a><a href={links.manifold}>Manifold Systems</a></nav></div><div className="shell footer-bottom"><span>An open-source project by Manifold Systems.</span><span>TASK → VERDICT + EXIT CODE</span></div></footer>;
}
